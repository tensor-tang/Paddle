...
uint32_t fmix32(uint32_t h) { return h; }
...
int bloomfilter_load_32bits(struct bloomfilter **bloomfilter, FILE *fp) {
  if (fp == NULL) {
    return 0;
  }
...
int bloomfilter_load(struct bloomfilter **bloomfilter, const void *path) {
  struct bloomfilter *t;
...
int bloomfilter_set(struct bloomfilter *bloomfilter, const void *key,
                    size_t len) {
...
int bloomfilter_set_nocheck(struct bloomfilter *bloomfilter, const void *key,
                            size_t len) {
  uint32_t i;
  uint64_t result[2];
  for (i = 0; i < bloomfilter->k; i++) {
...
int bloomfilter_get_hash(struct bloomfilter *bloomfilter, const void *key,
                         size_t len, char *dst) {
  uint32_t i;
  uint64_t result[2];
  char hash[255] = "";
...
int bloomfilter_dump(struct bloomfilter *bloomfilter, const void *path) {
 ...
 */
uint32_t char_to_little_endian_32bits(unsigned char *bytes) {
 ...
 * works either big-endian or little-endian architectures
 */
uint64_t char_to_little_endian_64bits(unsigned char *bytes) {
  uint64_t bytes_ull[8];
  int i;
  for (i = 0; i < 8; i++) {
    bytes_ull[i] = bytes[i];
  }
  return bytes_ull[0] | (bytes_ull[1] << 8) | (bytes_ull[2] << 16) |
         (bytes_ull[3] << 24) | (bytes_ull[4] << 32) | (bytes_ull[5] << 40) |
         (bytes_ull[6] << 48) | (bytes_ull[7] << 56);
}
}  // namespace math
}  // namespace operators
}  // namespace paddle
