...
TEST(FusedHashPass, basic) {
  auto prog = BuildProgramDesc();

  std::unique_ptr<ir::Graph> graph(new ir::Graph(prog));

  auto pass = PassRegistry::Instance().Get("fh_fuse_pass");
  ...
  // Assert fh op in newly generated graph
  int fh_count = 0;

  for (auto* node : graph->Nodes()) {
    if (node->IsOp() && node->Op()->Type() == "fh") {
      ++fh_count;
    }
  }
  EXPECT_EQ(fh_count, 1);
}
