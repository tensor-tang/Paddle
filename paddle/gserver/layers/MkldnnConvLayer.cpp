...
void MkldnnConvLayer::resetDnnFwd(PassType passType) {
  ...
  for (size_t i = 0; i != inputLayers_.size(); ++i) {
    ...
    // init cvt
    dataBot_->initIntlCvt(convPD->src_primitive_desc(), dnnCvtUser2Intl);
    if (usePaddleFmt_) {
      ...
      if (dataWgt_->initIntlCvt(
        convPD->weights_primitive_desc(), dnnCvtUser2Intl)) {
    ...
    if (hasBias) {
      ...
      if (dataBias_->initIntlCvt(
        convPD->bias_primitive_desc(), dnnCvtUser2Intl)) {
    ...
    if (dataTop_->initIntlCvt(convPD->dst_primitive_desc(), dnnCvtIntl2User)) {
  ...
  for (size_t i = 0; i != inputLayers_.size(); ++i) {
    ...
    if (weights_[i]->getWGrad()) {
      ...
      if (usePaddleFmt_) {
        ...
        if (diffWgt_->initIntlCvt(
          dataWgt_->getIntlPD(), dnnCvtIntl2User)) {
    ...
    if (hasBias) {
      ...
      if (diffBias_->initIntlCvt(bwdWgtPD->diff_bias_primitive_desc(),
        dnnCvtIntl2User)) {
...
//  MatrixPtr biases =Matrix::create(1,
//    biases_->getWGrad()->getElementCnt(), false, useGpu_);
...
//LOG(INFO) << "--------------------ex bias diff: "<< biases->getData()[0] << "," << biases->getData()[2];
...
void MkldnnConvLayer::submitBwdData(
  int inputIdx, const MatrixPtr& botGrad) {
  if (botGrad == NULL) {
    return;
  }
...
void MkldnnConvLayer::submitBwdWgts(
  int inputIdx, const MatrixPtr& botVal) {
  real* botdata = botVal->getData();  
  real* topdiff = getOutputGrad()->getData();
...
void MkldnnConvLayer::submitDnnBwd(const UpdateCallback &callback) {
  ...
  // cvt topdiff from user to intl only once
  MatrixPtr topGrad = getOutputGrad();
  std::vector<primitive> cvt;
  diffTop_->submitCvt(cvt, topGrad->getData());
//  LOG(INFO) << "cvt size:" << cvt.size();
  stream(stream::kind::eager).submit(cvt).wait();
