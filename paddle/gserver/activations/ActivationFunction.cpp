...
public:
  ...
  void resetDnnFwd(const Argument& arg, std::shared_ptr<void> topDataMD) {
    ...
    botData_.reset(new mkldnn::memory({*md, eg}, pdata));
    topData_.reset(new mkldnn::memory({*md, eg}, pdata));

    // careful: should be -0, not 0
    negative_slope = -0.f;

    auto fwdMD = mkldnn::relu_forward::desc(
      mkldnn::prop_kind::forward_training, *md, negative_slope);
  ...
  void resetDnnBwd(const Argument& arg, std::shared_ptr<void> topDiffMD) {
    ...
    std::shared_ptr<mkldnn::memory::desc> md =
        std::static_pointer_cast<mkldnn::memory::desc> (topDiffMD);
    botDiff_.reset(new mkldnn::memory({*md, eg}, pdiff));
    topDiff_.reset(new mkldnn::memory({*md, eg}, pdiff));

    auto bwdMD = mkldnn::relu_backward::desc(*md, *md, negative_slope);
...
public:
  ...
  void resetDnnFwd(const Argument& arg, std::shared_ptr<void> topDataMD) {
    ...
    auto fwdMD = mkldnn::softmax_forward::desc(
      mkldnn::prop_kind::forward_scoring, *md, axis);
